...
if (typeof window.SupabaseQueries === 'undefined') {
    class SupabaseQueries {
        ...
        async createPost(postData) {
            if (!this.isAvailable) {
                console.warn('createPost: Supabase client not available');
            ...
            try {
                const insertData = {
                    ...
                    approval_status: 'pending' // デフォルトで承認待ちに設定
                };

                if (window.CONFIG?.APP?.ENABLE_CONSOLE_LOG) {
                    console.log('createPost: Inserting post data:', insertData);
                }
                ...
                if (window.CONFIG?.APP?.ENABLE_CONSOLE_LOG) {
                    console.log('createPost: Successfully created post:', data);
                }
        ...
        async updateStudentPassword(payload) {
            if (!this.isAvailable) {
                console.warn('updateStudentPassword: Supabase client not available');
                return { data: null, error: null };
            }

            try {
                if (window.CONFIG?.APP?.ENABLE_CONSOLE_LOG) {
                    console.log('updateStudentPassword: Updating password for student:', payload.student_number);
                }
                ...
                if (error) {
                    console.error('updateStudentPassword: Database error:', error);
                    return { data: null, error };
                }

                if (window.CONFIG?.APP?.ENABLE_CONSOLE_LOG) {
                    console.log('updateStudentPassword: Successfully updated password for:', payload.student_number);
                }

                return { data: data?.[0] || null, error: null };
            } catch (error) {
                console.error('updateStudentPassword: Exception caught:', error);
                return { data: null, error };
            }
        }
