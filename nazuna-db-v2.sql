...
ALTER致以 posts ENABLE ROW LEVEL SECURITY;
...
-- 管理者はすべての投稿を閲覧・編集可能（adminロールが必要）
...
-- post_action_logsのポリシー
DROP POLICY IF EXISTS "Admins can view action logs" ON post_action_logs;
CREATE POLICY "Admins can view action logs" ON post_action_logs
    FOR SELECT USING (auth.role() = 'admin');

DROP POLICY IF EXISTS "Admins can create action logs" ON post_action_logs;
CREATE POLICY "Admins can create action logs" ON post_action_logs
    FOR INSERT WITH CHECK (auth.role() = 'admin');

-- chatsのポリシー
DROP POLICY IF EXISTS "Users can read chats for approved posts" ON chats;
CREATE POLICY "Users can read chats for approved posts" ON chats
    FOR SELECT USING (
        EXISTS (
            SELECT 1 FROM posts WHERE posts.id = chats.post_id AND posts.approval_status = 'approved'
        )
    );
